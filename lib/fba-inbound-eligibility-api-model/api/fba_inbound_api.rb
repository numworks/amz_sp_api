...
module AmzSpApi::FbaInboundEligibilityApiModel
  class FbaInboundApi
    ...
    def get_item_eligibility_preview_with_http_info(asin, program, opts = {})
      ...
      # verify enum value
      allowable_values = ["INBOUND", "COMMINGLING"]
      if @api_client.config.client_side_validation && !allowable_values.include?(program)
        fail ArgumentError, "invalid value for \"program\", must be one of #{allowable_values}"
      end
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'GetItemEligibilityPreviewResponse'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"FbaInboundApi.get_item_eligibility_preview",
