...
begin
  ...
  puts "Error when calling SalesApi->get_order_metrics: #{e}"
end
```

#### Using the get_order_metrics_with_http_info variant

This returns an Array which contains the response data, status code and headers.

> <Array(<GetOrderMetricsResponse>, Integer, Hash)> get_order_metrics_with_http_info(marketplace_ids, interval, granularity, opts)

```ruby
begin
  
  data, status_code, headers = api_instance.get_order_metrics_with_http_info(marketplace_ids, interval, granularity, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <GetOrderMetricsResponse>
rescue AmzSpApi::SalesApiModel::ApiError => e
  puts "Error when calling SalesApi->get_order_metrics_with_http_info: #{e}"
end
```
