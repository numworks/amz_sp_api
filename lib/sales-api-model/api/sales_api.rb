...
module AmzSpApi::SalesApiModel
  class SalesApi
    ...
    # @param marketplace_ids [Array<String>] A marketplace identifier. This specifies the marketplace in which the order was placed. Only one marketplace can be specified.  For example, ATVPDKIKX0DER indicates the US marketplace.
    # @param interval [String] A time interval used for selecting order metrics. This takes the form of two dates separated by two hyphens (first date is inclusive; second date is exclusive). Dates are in ISO8601 format and must represent absolute time (either Z notation or offset notation). Example: 2018-09-01T00:00:00-07:00--2018-09-04T00:00:00-07:00 requests order metrics for Sept 1st, 2nd and 3rd in the -07:00 zone.
    ...
    def get_order_metrics_with_http_info(marketplace_ids, interval, granularity, opts = {})
      ...
      # verify enum value
      allowable_values = ["Hour", "Day", "Week", "Month", "Year", "Total"]
      ...
      allowable_values = ["B2B", "B2C", "All"]
      if @api_client.config.client_side_validation && opts[:'buyer_type'] && !allowable_values.include?(opts[:'buyer_type'])
        fail ArgumentError, "invalid value for \"buyer_type\", must be one of #{allowable_values}"
      end
      allowable_values = ["Monday", "Sunday"]
      if @api_client.config.client_side_validation && opts[:'first_day_of_week'] && !allowable_values.include?(opts[:'first_day_of_week'])
        fail ArgumentError, "invalid value for \"first_day_of_week\", must be one of #{allowable_values}"
      end
      # resource path
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'GetOrderMetricsResponse'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"SalesApi.get_order_metrics",
