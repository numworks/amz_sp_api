...
module AmzSpApi::ServicesApiModel
  ...
  class Poa
    ...
    def list_invalid_properties
      ...
      if !@technicians.nil? && @technicians.length < 1
        invalid_properties.push('invalid value for "technicians", number of items must be greater than or equal to 1.')
      end

      pattern = Regexp.new(/^[A-Z0-9]*$/)
      if !@uploading_technician.nil? && @uploading_technician !~ pattern
        invalid_properties.push("invalid value for \"uploading_technician\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      return false if !@technicians.nil? && @technicians.length < 1
    ...
    # @param [Object] technicians Value to be assigned
    def technicians=(technicians)
      if !technicians.nil? && technicians.length < 1
        fail ArgumentError, 'invalid value for "technicians", number of items must be greater than or equal to 1.'
      end
    ...
    def uploading_technician=(uploading_technician)
      pattern = Regexp.new(/^[A-Z0-9]*$/)
      if !uploading_technician.nil? && uploading_technician !~ pattern
        fail ArgumentError, "invalid value for \"uploading_technician\", must conform to the pattern #{pattern}."
      end
