...
module AmzSpApi::ProductTypeDefinitionsApiModel
  class DefinitionsApi
    ...
    # @param marketplace_ids [Array<String>] A comma-delimited list of Amazon marketplace identifiers for the request. Note: This parameter is limited to one marketplaceId at this time.
    ...
    def get_definitions_product_type_with_http_info(product_type, marketplace_ids, opts = {})
      ...
      allowable_values = ["LISTING", "LISTING_PRODUCT_ONLY", "LISTING_OFFER_ONLY"]
      if @api_client.config.client_side_validation && opts[:'requirements'] && !allowable_values.include?(opts[:'requirements'])
        fail ArgumentError, "invalid value for \"requirements\", must be one of #{allowable_values}"
      end
      allowable_values = ["ENFORCED", "NOT_ENFORCED"]
      if @api_client.config.client_side_validation && opts[:'requirements_enforced'] && !allowable_values.include?(opts[:'requirements_enforced'])
        fail ArgumentError, "invalid value for \"requirements_enforced\", must be one of #{allowable_values}"
      end
      allowable_values = ["DEFAULT", "ar", "ar_AE", "de", "de_DE", "en", "en_AE", "en_AU", "en_CA", "en_GB", "en_IN", "en_SG", "en_US", "es", "es_ES", "es_MX", "es_US", "fr", "fr_CA", "fr_FR", "it", "it_IT", "ja", "ja_JP", "nl", "nl_NL", "pl", "pl_PL", "pt", "pt_BR", "pt_PT", "sv", "sv_SE", "tr", "tr_TR", "zh", "zh_CN", "zh_TW"]
      ...
      # resource path
      local_var_path = '/definitions/2020-09-01/productTypes/{productType}'.sub('{' + 'productType' + '}', CGI.escape(product_type.to_s))
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'ProductTypeDefinition'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"DefinitionsApi.get_definitions_product_type",
    ...
    def search_definitions_product_types_with_http_info(marketplace_ids, opts = {})
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'ProductTypeList'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"DefinitionsApi.search_definitions_product_types",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
