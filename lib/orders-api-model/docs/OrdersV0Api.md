...
begin
  ...
  puts "Error when calling OrdersV0Api->get_orders: #{e}"
end
```
...
> <Array(<GetOrdersResponse>, Integer, Hash)> get_orders_with_http_info(marketplace_ids, opts)

```ruby
begin
  
  data, status_code, headers = api_instance.get_orders_with_http_info(marketplace_ids, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <GetOrdersResponse>
rescue AmzSpApi::OrdersApiModel::ApiError => e
  puts "Error when calling OrdersV0Api->get_orders_with_http_info: #{e}"
end
```
...
#### Using the update_verification_status_with_http_info variant
