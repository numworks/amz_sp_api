  ...
  class ItemApprovalContext
    ...
    def self.openapi_types
      {
        ...
        :'approval_support_data' => :'Array<ApprovalSupportDataElement>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::OrdersApiModel::ItemApprovalContext` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::OrdersApiModel::ItemApprovalContext`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'approval_support_data')
        if (value = attributes[:'approval_support_data']).is_a?(Array)
          self.approval_support_data = value
        end
      end
    end
